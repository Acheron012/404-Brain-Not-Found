...
import { Task, TaskStatus, Intensity } from '../types';
...
export function TaskModal({ isOpen, onClose, date, tasks, onDeleteTask, onUpdateTask }: TaskModalProps) {
  ...
  const dateTasks = tasks.filter(t => t.deadline === dateString);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-[#F4F7F5] rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between p-6 border-b border-[#BFD8B8] bg-[#E3EFE6]">
          <div>
            <h3 className="text-xl font-bold text-[#2F3E34]">Tasks for {date.toLocaleDateString()}</h3>
            <p className="text-sm text-[#2F3E34]/70 mt-1">{dateTasks.length} task{dateTasks.length !== 1 && 's'} scheduled</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {dateTasks.length === 0 ? (
            ...
            <div className="space-y-4">
              {dateTasks.map(task => (
                <div key={task.id} className="p-4 border border-[#BFD8B8] rounded-xl bg-white shadow-sm hover:border-[#7FB77E] transition-colors">
                  {editingId === task.id ? (
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex gap-1 ml-4">
                          <button onClick={() => startEdit(task)} className="p-1.5 text-[#2F3E34]/40 hover:text-[#7FB77E] hover:bg-[#E3EFE6] rounded transition-colors">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => onDeleteTask(task.id)} className="p-1.5 text-[#2F3E34]/40 hover:text-red-500 hover:bg-red-50 rounded transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </div>
                      <p className="text-sm text-[#2F3E34]/70 mb-3">{task.description}</p>
